...
const primaryItems: AdminNavLink[] = [
  { key: "hub", href: "/admin", label: "Accueil" },
...
const groups: AdminNavGroup[] = [
  ...
  },
  {
    ...
    keys: ["mockups"],
    items: [
      { activeKey: "mockups", href: "/admin/mockups/answer-cards", label: "Réponses" },
      { activeKey: "mockups", href: "/admin/mockups/tier-breakdown", label: "Tiers" },
    ],
  },
  {
    label: "Design",
    keys: ["fx-lab", "brand-system"],
    items: [
...
const trailingItems: AdminNavLink[] = [
  { href: "/", label: "ManabuPlay" },
];
...
export function getAdminNavHtml(active: AdminNavKey) {
  ...
  const groupLinks = groups
    ...
    .join("");
  const trailingLinks = trailingItems
    .map((item) => `<a class="admin-topnav__link" href="${escapeAttr(item.href)}">${item.label}</a>`)
    .join("");

  return `<nav class="admin-topnav">${links}${groupLinks}<span class="admin-topnav__spacer"></span>${trailingLinks}<div class="admin-topnav__lang" aria-label="Language switch"><button class="admin-topnav__lang-btn" type="button" data-admin-lang="en">EN</button><button class="admin-topnav__lang-btn" type="button" data-admin-lang="fr">FR</button></div></nav><script>(function(){const KEY='mp_lang';const root=document.documentElement;const buttons=[...document.querySelectorAll('[data-admin-lang]')];const groups=[...document.querySelectorAll('.admin-topnav__group')];function closeGroups(except){groups.forEach((group)=>{if(group!==except)group.removeAttribute('open');});}groups.forEach((group)=>{group.addEventListener('toggle',()=>{if(group.open)closeGroups(group);});group.querySelectorAll('a').forEach((link)=>link.addEventListener('click',()=>group.removeAttribute('open')));});document.addEventListener('click',(event)=>{if(!event.target.closest('.admin-topnav__group'))closeGroups();});document.addEventListener('keydown',(event)=>{if(event.key==='Escape')closeGroups();});function readLang(){try{const raw=localStorage.getItem(KEY);return raw?JSON.parse(raw):'en';}catch(_){return'en';}}function writeLang(lang){try{localStorage.setItem(KEY,JSON.stringify(lang));}catch(_){}}function emit(lang){window.dispatchEvent(new CustomEvent('adminlangchange',{detail:{lang}}));}function apply(lang){root.lang=lang==='fr'?'fr':'en';buttons.forEach((button)=>button.classList.toggle('is-active',button.getAttribute('data-admin-lang')===lang));emit(root.lang);}const initial=readLang();apply(initial);buttons.forEach((button)=>button.addEventListener('click',()=>{const lang=button.getAttribute('data-admin-lang')||'en';writeLang(lang);apply(lang);}));})();</script>`;
}
