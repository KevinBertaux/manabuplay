...
const LS = {
  get(k) {
    try {
      return JSON.parse(localStorage.getItem("mp_" + k));
    } catch (e) {
      return null;
    }
  },
...
function launchQuiz() {
  ...
  state = {
    questions: buildQuestions(currentDiff.words),
    currentIndex: 0,
    score: 0,
    streak: 0,
    bestStreak: 0,
...
function spawnParticles(btn, color) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    p.style.cssText = `left:${cx}px;top:${cy}px;background:${color};--tx:${(Math.random() - 0.5) * 120}px;--ty:${(Math.random() - 0.8) * 100}px;`;
...
async function handleEmailSubmit(e) {
  ...
  if (!input.validity.valid || !isValidWaitlistEmail(email)) {
    input.setCustomValidity(
      currentLang === "fr" ? "Entre une adresse email valide." : "Enter a valid email address.",
    );
...
function copyShareLink() {
  ...
  navigator.clipboard
    ...
    .catch(() => {
      ...
      window.prompt(
        currentLang === "fr" ? "Copie ce texte manuellement :" : "Copy this text manually:",
        text,
      );
    });
}
