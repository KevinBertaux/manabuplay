import type { RoadmapItem, RoadmapPayload, RoadmapVersion } from "../lib/admin-roadmap";
...
if (backlogList && currentVersionChip && summaryItemsChip && summaryChildrenChip) {
  void (async () => {
    const response = await fetch("/internal/roadmap-payload.json");
    if (!response.ok) {
      throw new Error(`Failed to load roadmap payload: ${response.status}`);
    }

    const payload = (await response.json()) as RoadmapPayload;
